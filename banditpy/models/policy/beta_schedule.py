...
from .base import BoundsRegistry, ParameterSpec
...
class BetaSchedule:
    ...
    Subclasses declare their free parameters via the ``parameters`` class
    variable (same ``ParameterSpec`` convention as ``BasePolicy``).
    Bounds are accessible and editable via ``self.bounds``.

    Examples
    --------
    model = DecisionModel(task, Qlearn2Arm())
    model = DecisionModel(task, Qlearn2Arm(), beta_schedule=ExponentialBeta())
    model.beta_schedule.bounds["beta_0"] = (0.5, 20.0)
    """
    ...
    def get_bounds(self) -> Dict[str, Tuple]:
        return {k: p.bounds for k, p in self._param_specs.items()}
...
class ExponentialBeta(BetaSchedule):
    """
    Exponentially changing beta: ``beta_0 * exp(beta_rate * t)``.
    ...
    Notes
    -----
    Positive ``beta_rate`` → beta grows (increasingly greedy over a session).
    Negative ``beta_rate`` → beta shrinks (increasingly exploratory).
    The default bounds ``(-0.02, 0.02)`` are tight to prevent explosion;
    widen with ``model.beta_schedule.bounds["beta_rate"] = (-0.1, 0.1)``.
    """
    ...
    parameters = [
        ...
        ParameterSpec(
            "beta_rate",
            (-0.02, 0.02),
            description="Exponential growth rate of beta per trial",
        ),
        ParameterSpec(
            "epsilon",
            (0.0, 0.3),
